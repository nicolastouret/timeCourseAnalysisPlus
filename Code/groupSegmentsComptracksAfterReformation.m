    ...
    if lengthSeqOfEvents > 2
        ...
        for indexGroup=1:length(compTrackSegCell)
            ...
            if any (segmentGroup==1)
                ...
                %increase iTrackNew, because if there are multiple
                %non-interacting tracks in this compound track, we put
                %track #1 in corresponding row of tracksReform output while
                %track #2,3,4... and so on are padded as new rows at the
                %end of tracksReform. - comment TN 20191001
            ...
            timeStartOri=min(compTracks(iTrack).seqOfEvents(:,1));
            timeStartNew=min(tracksReform( iTrackCurr).seqOfEvents(:,1));
            
            % calculate the end times
            
            timeEndOri=max(compTracks(iTrack).seqOfEvents(:,1));
            timeEndNew=max(tracksReform( iTrackCurr).seqOfEvents(:,1));
